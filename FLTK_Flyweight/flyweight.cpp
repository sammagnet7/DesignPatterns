...
/*Flyweight factory for returning static Flyweight objects */
class BallFlyweightFactory
{
public:
    static BallFlyweight *getBallType(BALL_TYPE type)
    {
        static std::unordered_map<int, BallFlyweight *> ballType_cache = //
            {
                {BEACHBALL, new BallFlyweight("ball_beach.png", 25)},
                {BASKETBALL, new BallFlyweight("ball_basket.png", 25)},
                {SOCCERBALL, new BallFlyweight("ball_soccer.png", 25)} //
            };
        return ballType_cache.at(type);
    }
};
...
public:
    ...
    {
        BallFlyweight *ballState = BallFlyweightFactory::getBallType(current_type);
